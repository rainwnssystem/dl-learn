## Deep Learning

Deep Learning에 대해 학습 후 실습한 자료들을 정리합니다.

### Contents
* PyTorch
  * Classification
    * [DNN - MNIST 숫자 분류](./dnn_pytorch_mnist.ipynb)
    * [DNN - CIFAR-10 물체 분류](./dnn_pytorch_cifar10.ipynb)
    * [CNN - MNIST 숫자 분류](./cnn_pytorch_mnist.ipynb)
    * [CNN - CIFAR-10 물체 분류](./cnn_pytorch_cifar10.ipynb)
    * [CNN - Pizza 분류](./cnn_pytorch_mnist.ipynb)
    * [RNN - MNIST 숫자 분류](./rnn_pytorch_mnist.ipynb)
    * [LSTM - MNIST 숫자 분류](./lstm_pytorch_mnist.ipynb)
    * [RNN - IMDB 긍정/부정 분류](./rnn_pytorch_imdb.ipynb)
    * [CNN - Chest X-ray 폐렴 여부 분류](./cnn_pytorch_xray.ipynb)
    * [CNN - Cats-vs-Dogs 동물 분류](./cnn_pytorch_cats_dogs.ipynb)
  * Regression
    * [RNN - Stock 주가 예측](./rnn_pytorch_stock.ipynb)
    * [LSTM - Stock 주가 예측](./lstm_pytorch_stock.ipynb)
* Keras
  * Classification
    * [DNN - MNIST 숫자 분류](./dnn_keras_mnist.ipynb)
    * [CNN - CIFAR-10 물체 분류](./cnn_keras_cifar10.ipynb)
    * [CNN - Pizza 분류](./cnn_keras_pizza.ipynb)
    * [RNN - MNIST 숫자 분류](./rnn_keras_mnist.ipynb)
    * [LSTM - MNIST 숫자 분류](./lstm_keras_mnist.ipynb)
* SageMaker AI
  * Classification
    * [CNN - CIFAR-10 물체 분류](./cnn_sagemaker_cifar10.ipynb)
    * [DNN - MNIST 숫자 분류](./dnn_sagemaker_mnist.ipynb)


### Datasets
* [MNIST](https://www.kaggle.com/datasets/hojjatk/mnist-dataset)
* [CIFAR-10](https://www.kaggle.com/datasets/swaroopkml/cifar10-pngs-in-folders)
* [Pizza or Not Pizza?](https://www.kaggle.com/datasets/carlosrunner/pizza-not-pizza)
* [TESLA Stock Data](https://www.kaggle.com/datasets/varpit94/tesla-stock-data-updated-till-28jun2021)
* [IMDB Reviews](https://www.kaggle.com/datasets/lakshmi25npathi/imdb-dataset-of-50k-movie-reviews)
* [Chest X-ray](https://www.kaggle.com/datasets/tolgadincer/labeled-chest-xray-images)
* [Cats-vs-Dogs](https://www.kaggle.com/datasets/shaunthesheep/microsoft-catsvsdogs-dataset)